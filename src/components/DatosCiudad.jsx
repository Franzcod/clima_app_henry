...
export default function DatosCiudad(props) {
  ...
  useEffect(() => {
    ...
    function getPhoto() {
      fetch(
        `https://api.unsplash.com/search/photos/?query=${props.ciudad}&client_id=${apiKey_3}`
      )
        .then((r) => r.json())
        .then((recurso) => {
          lugar = recurso.results[1].urls.small;
        });
    }
    ...
    function getInfoCity() {
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?q=${props.ciudad}&appid=${apiKey}&units=metric`
      )
        .then((r) => r.json())
        .then((recurso) => {

...
export default function PrincipalComp() {
  ...
  React.useEffect(() => {
    if (navigator.geolocation) {
      ...
      //   console.log("bebe");
      // }
      // console.log(navigator.geolocation.watchPosition(_,_,{timeout:10000}));
      ...
      }, setData({ name: "Sin ubicacion" })); // <<==  Si no tiene la ubicacion
    }
  }, [apiKey]);
  ...
  async function onSearch(ciudad) {
    ...
    //
    await fetch(
      `https://api.unsplash.com/search/photos/?query=${ciudad}&client_id=${apiKey_3}`
    )
      .then((r) => r.json())
      .then((recurso) => {
        console.log(recurso);
